...
function PrintFilterModalView(props) {
  const [defaultRowKeys, setDefaultRowKeys] = useState(
    props.filteredPrintItem.map((item) => item.id)
  );
  const rowSelection = {
  ...
  const setItems = () => {
    props.setFilteredPrintItem(
      props.filteredPrintItem.filter((item) => defaultRowKeys.includes(item.id))
    );

    props.setSelectedItems(
      props.selectedItems.map((item) => {
        if (item.id === props.filteredPrintItem[0].groupId) {
          return {
            ...item,
            items: props.filteredPrintItem.filter((item) =>
    ...
    <Modal>
      <ModalContainer>
        ...
        <Table
          ...
          columns={columns}
          dataSource={props.filteredPrintItem}
        />
        <ButtonWrapper>
...
`;
export default PrintFilterModalView;
